local mftj = "https://raw.githubusercontent.com/ParKe001/ParKe/refs/heads/main/Window1.lua.txt"
...
local J = F:CreateWindow({
    Title = "<font color='#FF0000'>YLQ</font>script Hub",
    Folder = wj,
    Icon = xb,
    IconThemed = false,
    IconSize = 10*2,
...
local K = Color3.fromHex("#7775F2")
local L = Color3.fromHex("#ECA201")
local M = Color3.fromHex("#10C550")
local N = Color3.fromHex("#83889E")
local O = Color3.fromHex("#257AF7")
local P = Color3.fromHex("#EF4F1D")
...
local function Q(R, S, T, U)
    ...
    if X == "table" then
        ...
        for _, _ in pairs(R) do
            if type(_) == "number" and _ > Z then
                Z = _
            end
            if type(_) ~= "number" or _ <= 0 or math.floor(_) ~= _ then
                Y = false
                break
            end
        end
        ...
        if Y then
            ...
            local a3 = true
            local a4 = {}
            for a5 in pairs(R) do
                table.insert(a4, a5)
            end
            table.sort(a4, function(a6, a7)
                if type(a6) == type(a7) then
                    return tostring(a6) < tostring(a7)
                else
                    return type(a6) < type(a7)
                end
            end)
            for _, a5 in ipairs(a4) do
                local a8 = R[a5]
                if not a3 then
                    a1 = a1 .. ",\n"
                else
